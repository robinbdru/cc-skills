...
Use the explanation as a map, but judge from the code itself — the explanation may have framed things charitably. Find architectural problems, not line-level bugs or style issues. The question is whether the subsystem is built well for what it needs to do and how it will need to evolve.
...
1. **Severity**
   ...
   - `observation`: a tradeoff that might not age well, a pattern inconsistent with the rest of the codebase, technical debt.
2. **Components**: which parts of the system are involved.
3. **Finding**: the issue, specifically. Name the boundary, the coupling, the component.
4. **Evidence**: concrete code. Don't assert "this is too coupled" — show the dependency chain.
5. **Impact**: what it costs. Harder to test? Harder to change? Performance cliff at scale?

Avoid: line-level review, rewrite proposals that don't demonstrate a problem with the current approach, "this could use more abstraction" without showing what the abstraction would solve, and flagging intentional tradeoffs that have clear benefits.
